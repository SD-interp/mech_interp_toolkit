...
def mock_config():
    ...
    config = Qwen2Config(num_attention_heads=4, num_hidden_layers=2, hidden_size=128)
...
def test_true_integrated_gradients(mock_config):
    """Tests the true_integrated_gradients function."""
    model = MagicMock()
    model.model.config = mock_config
    inputs = {"input_ids": torch.randint(0, 100, (1, 5))}
    baseline_embeddings = ActivationDict(mock_config, positions=slice(None))
    ...
    with patch(
        "mech_interp_toolkit.gradient_based_attribution.eap_integrated_gradients"
    ) as mock_eap_ig:
        mock_eap_ig.return_value = "test"
        result = true_integrated_gradients(model, inputs, baseline_embeddings, steps=10)

        assert result == "test"
        mock_eap_ig.assert_called_once()
        kwargs = mock_eap_ig.call_args.kwargs
        assert kwargs["layer_components"] == [(0, "layer_in")]
